  ...
  return (
    <>
      {isReaderMode && (
        <div ref={readerModeRef} className={`reader-mode fixed inset-0 z-[9999] ${isDarkReader ? 'bg-[#111111] reader-mode-dark' : 'bg-[#FFFBF0] reader-mode-light'} overflow-y-scroll`}
          ...
          <div className='mx-auto max-w-3xl px-6 sm:px-12 pt-10 pb-20 w-full'>
            {/* Top Controls Fixed */}
            <div className="flex justify-between items-center z-10 relative mb-8 sticky top-0 bg-inherit py-2">
              <button
            ...
            {/* Header Section */}
            <div className={`${isDarkReader ? 'text-gray-300' : 'text-gray-900'} font-serif`}>
              <p className={`font-medium text-[16px] mb-2 font-sans ${isDarkReader ? 'text-gray-400' : 'text-gray-500'}`}>{post.date}</p>
              <h1 className={`font-black md:text-[60px] sm:text-[50px] xs:text-[40px] text-[30px] leading-tight font-serif ${isDarkReader ? 'text-[#EAEAEA]' : 'text-[#2a2a2a]'}`}>
                {post.title}
              </h1>
            </div>
            ...
            {/* Main Content */}
            <div
              className={`prose prose-lg blog-article ${useDarkCodeTheme ? 'code-theme-dark' : 'code-theme-light'} max-w-none mt-8 transition-all duration-500 ${isDarkReader ? 'prose-invert text-[#D1D5DB]' : 'prose-slate text-[#333333]'}`}
              style={{ fontSize: '21px', lineHeight: '1.85' }}
      ...
      <motion.div
        ...
        className={`relative z-0 min-h-screen transition-colors duration-500 ${!isReaderMode ? 'bg-primary pt-20' : 'hidden'}`}>
        <div className={`mx-auto flex flex-col gap-10 ${`${styles.paddingX} max-w-5xl`}`}>
        ...
        <div>
          ...
          <p className='font-medium text-[16px] mb-2 font-sans text-secondary'>{post.date}</p>
          <h1 className='font-black md:text-[60px] sm:text-[50px] xs:text-[40px] text-[30px] leading-tight text-white'>
            {post.title}
          </h1>
        ...
        {/* Main Content */}
        <div
          className='prose prose-lg blog-article code-theme-dark max-w-none mb-20 transition-all duration-500 text-secondary text-[18px] leading-[32px] mt-4'
          dangerouslySetInnerHTML={{ __html: post.content }}
        />
      </div>
